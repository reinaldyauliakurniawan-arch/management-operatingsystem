...
class TeamController extends Controller
{
    ...
    public function index()
    {
        ...
        return Inertia::render('Teams/Index', [
            'teams'          => $teams,
            'allUsers'       => $allUsers,
            'allSystemUsers' => \App\Models\User::orderBy('name')->get(['id', 'name', 'email', 'is_org_admin', 'created_at']),
        ]);
    }
    ...
    public function users()
    {
        ...
        $users = \App\Models\User::orderBy('name')->get(['id', 'name', 'email', 'is_org_admin', 'created_at']);
        $teams = \App\Models\Team::where('organization_id', session('active_organization_id'))->get(['id', 'name']);
        $allUsers = $users;
        return inertia('Teams/Index', [
            'teams'    => \App\Models\Team::withoutGlobalScopes()
                ->with(['members.user'])
                ...
                ->map(fn($team) => [
                    ...
                    'parent_team_id' => $team->parent_team_id,
                    'member_count'   => $team->members->count(),
                    'leaders'        => $team->members->where('role', 'leader')->map(fn($m) => ['id' => $m->user_id, 'name' => $m->user->name])->values(),
                ]),
            'allUsers' => $allUsers,
