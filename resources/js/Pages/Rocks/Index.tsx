...
const statusBadgeVariant: Record<Rock["status"], "success" | "error" | "neutral"> = {
    on_track: "success",
    off_track: "error",
    done: "neutral",
};
...
const selectClass =
    "w-full bg-[#f0f0f0] border border-[#e4e4e4] rounded-lg px-3 py-2 text-sm text-[#1a1a1a] focus:outline-none focus:border-[#1a5c41] focus:ring-2 focus:ring-[#1a5c41]/10 transition-colors appearance-none";
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="mb-xl grid grid-cols-4 gap-lg">
                {[
                    { label: "Total", value: total, valueClass: "text-[#1a1a1a]" },
                    { label: "On Track", value: onTrack, valueClass: "text-[#1a5c41]" },
                    { label: "Off Track", value: offTrack, valueClass: "text-[#991b1b]" },
                    { label: "Done", value: done, valueClass: "text-[#6b6b6b]" },
                ].map((stat) => (
                    <Card key={stat.label}>
                        <CardContent className="pt-xl">
                            ...
                            <p className={`text-[32px] font-semibold tracking-tight leading-none ${stat.valueClass}`}>
                                {stat.value}
                            </p>
                        </CardContent>
                    </Card>
            ...
            <Table>
                <TableHeader>
                    <TableRow>
                        {["Rock", "Owner", "Periode", "Due Date", "Status", ""].map((h) => (
                    ...
                    {rockList.length === 0 && (
                        <TableRow>
                            <TableCell colSpan={6} className="py-12 text-center text-text-muted">
                                Belum ada rock. {isLeader && "Tambah rock pertama."}
                            </TableCell>
                        </TableRow>
                    )}
                    {rockList.map((rock) => (
                        <TableRow key={rock.id}>
                            ...
                            <TableCell className="text-text-secondary">{rock.owner.name}</TableCell>
                            <TableCell className="text-text-secondary">{rock.quarter} {rock.year}</TableCell>
                            <TableCell className="text-text-secondary">{rock.due_date ?? "—"}</TableCell>
                            <TableCell>
                                <Badge variant={statusBadgeVariant[rock.status]}>
                                ...
                                <div className="flex items-center justify-end gap-sm">
                                    {isLeader && (
                                        <select
                                            ...
                                            className="bg-[#f0f0f0] border border-[#e4e4e4] rounded-lg px-2 py-1 text-xs text-[#1a1a1a] focus:outline-none focus:border-[#1a5c41] appearance-none"
                                        >
                                    ...
                                    <Button variant="ghost" size="sm" onClick={() => setDetailRock(rock)}>
                                        Detail
                                    </Button>
                                    {isLeader && (
                                        <Button variant="destructive" size="sm" onClick={() => deleteRock(rock.id)}>
                                            Hapus
                                        </Button>
                                    )}
                                </div>
                    ...
                    <DialogBody>
                        <form onSubmit={submit} className="flex flex-col gap-lg">
                            <div className="flex flex-col gap-xs">
                                ...
                                />
                                {errors.title && (
                                    <p className="text-[12px] text-error-text">{errors.title}</p>
                                )}
                            </div>
                            <div className="flex flex-col gap-xs">
            ...
            <Dialog open={!!detailRock} onOpenChange={(open) => !open && setDetailRock(null)}>
                <DialogContent size="md">
                    <DialogHeader>
                        ...
                        <>
                            <DialogBody className="flex flex-col gap-xl">
                                <div className="flex flex-wrap items-center gap-md">
                                    <Badge variant={statusBadgeVariant[detailRock.status]}>
                                        {statusLabel[detailRock.status]}
                                    </Badge>
                                    <span className="text-[12px] text-text-muted">
                                        {detailRock.owner.name} · {detailRock.quarter} {detailRock.year}
                                    </span>
                                    {detailRock.due_date && (
                                        <span className="text-[12px] text-text-muted">
                                ...
                                {detailRock.description && (
                                    <p className="text-sm text-text-secondary">{detailRock.description}</p>
                                )}
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-col gap-sm">
                                        {detailRock.milestones.length === 0 && (
                                            <p className="text-[13px] text-text-muted">Belum ada milestone.</p>
                                        )}
                                        {detailRock.milestones.map((m) => (
                                            ...
                                            >
                                                <input
                                                    ...
                                                    className="h-4 w-4 cursor-pointer accent-[#1a5c41]"
                                                />
                                                <span className={`flex-1 text-[13px] ${m.is_done ? "text-text-muted line-through" : "text-text-primary"}`}>
                                                    {m.title}
                                                </span>
                                                {m.due_date && (
                                                    <span className="text-[11px] text-text-muted">{m.due_date}</span>
                                                )}
                                                {isLeader && (
                                                    <Button
                                                        variant="ghost"
                                    ...
                                    {isLeader && (
                                        <div className="mt-md flex gap-sm">
                                            <Input
                                                ...
                                                onKeyDown={(e) => e.key === "Enter" && addMilestone(detailRock.id)}
                                                className="flex-1"
                                            />
                                            <Button onClick={() => addMilestone(detailRock.id)}>
                                                Tambah
                                            </Button>
                                        </div>
                                    )}
                            ...
                            {isLeader && (
                                <DialogFooter>
                                    <Button variant="destructive" onClick={() => deleteRock(detailRock.id)}>
                                        Hapus Rock
                                    </Button>
                                </DialogFooter>
                            )}
