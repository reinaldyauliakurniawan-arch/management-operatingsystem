...
const DB_NAME = process.env.DB_NAME || process.env.DB_DATABASE; // required, no sane default

if (!DB_NAME) {
    console.error(
        "Missing DB_NAME env var. Set it to your Laravel app's database name.",
    );
    process.exit(1);
}
...
async function assertAllowedTable(table) {
    ...
    if (!all.includes(table)) {
        throw new Error(
            `Table "${table}" does not exist. Use list_tables to see available tables.`,
        );
    }
    if (DENYLIST.has(table)) {
        throw new Error(
            `Table "${table}" is not accessible through this server (sensitive/internal table).`,
        );
    }
    return table;
}
...
async function sanitizeData(table, data, { forCreate }) {
    ...
    for (const [key, value] of Object.entries(data || {})) {
        ...
        if (!columnNames.has(key)) {
            throw new Error(
                `Unknown column "${key}" on table "${table}". Use describe_table to see valid columns.`,
            );
        }
...
function buildServer() {
    ...
    server.registerTool(
        ...
        {
            description:
                "List all tables available for CRUD (sensitive tables like users/sessions are hidden).",
            inputSchema: {},
        },
        async () => {
            const tables = (await listAllTableNames()).filter(
                (t) => !DENYLIST.has(t),
            );
            return {
                content: [
    ...
    server.registerTool(
        ...
        {
            description:
                "Show columns (name, type, nullable, default, primary key) for a table.",
        ...
        async ({ table }) => {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify(await getColumns(table), null, 2),
                    },
                ],
            };
        },
    ...
    server.registerTool(
        ...
        {
            description:
                "List rows from a table, optionally filtered by exact-match column values. Supports pagination.",
            inputSchema: {
                table: z.string(),
                filters: z
                    .record(
        ...
        async ({ table, filters, limit, offset }) => {
            ...
            const columns = new Set(
                (await getColumns(table)).map((c) => c.name),
            );
            const whereParts = [];
            const params = [];
            for (const [key, value] of Object.entries(filters || {})) {
                if (!columns.has(key))
                    throw new Error(
                        `Unknown filter column "${key}" on table "${table}".`,
                    );
            ...
            const whereSql = whereParts.length
                ...
                : "";
            const [rows] = await pool.query(
                `SELECT * FROM ${q(table)} ${whereSql} LIMIT ? OFFSET ?`,
                [...params, limit, offset],
            );
            return {
                content: [
    ...
    server.registerTool(
        "get_record",
        {
            description: "Get a single row by primary key id.",
            inputSchema: {
                table: z.string(),
        ...
        async ({ table, id }) => {
            await assertAllowedTable(table);
            const pk =
                (await getColumns(table)).find((c) => c.key === "PRI")?.name ||
                "id";
            const [rows] = await pool.query(
                `SELECT * FROM ${q(table)} WHERE ${q(pk)} = ?`,
                [id],
            );
            if (!rows[0])
                return {
                    content: [
                        {
                            type: "text",
                            text: `No row found in "${table}" with id ${id}.`,
                        },
                    ],
                };
            return {
    ...
    server.registerTool(
        ...
        {
            description:
                "Insert a row. 'data' keys must match real columns. created_at/updated_at auto-filled if omitted.",
            inputSchema: { table: z.string(), data: z.record(z.any()) },
        },
        async ({ table, data }) => {
            await assertAllowedTable(table);
            ...
            if (keys.length === 0)
                throw new Error("No valid columns provided in 'data'.");
    ...
    server.registerTool(
        ...
        {
            description:
                "Update a row by id. Only given columns change. updated_at refreshed automatically.",
            inputSchema: {
                table: z.string(),
                id: z.union([z.number(), z.string()]),
                data: z.record(z.any()),
    ...
    server.registerTool(
        ...
        {
            description:
                "Delete by id. Soft-deletes if table has 'deleted_at' (Laravel SoftDeletes), unless hard=true.",
            inputSchema: {
                table: z.string(),
                id: z.union([z.number(), z.string()]),
        ...
        async ({ table, id, hard }) => {
                ...
                if (info.affectedRows === 0)
                    return {
                        content: [
                            {
                                type: "text",
                                text: `No active row found in "${table}" with id ${id}.`,
                            },
                        ],
                    };
                return {
                    content: [
                        {
                            type: "text",
                            text: `Soft-deleted row ${id} from "${table}".`,
                        },
                    ],
                };
            }
            ...
            const [info] = await pool.query(
                `DELETE FROM ${q(table)} WHERE ${q(pk)} = ?`,
                [id],
            );
            if (info.affectedRows === 0)
                return {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Permanently deleted row ${id} from "${table}".`,
                    },
                ],
            };
        },
...
app.get("/", (req, res) => {
    res.json({ name: "management-os-mcp", version: "1.0.0" });
});
...
app.get("/health", async (req, res) => {
    try {
        res.json({
            ok: true,
            database: DB_NAME,
            tableCount: (await listAllTableNames()).length,
        });
    } catch (err) {
